...
Phase 0 of the run_cli/run_agent/_run_research unification (see
~/.claude/plans/polymorphic-wibbling-fiddle.md and session_status/CURRENT.md, 2026-09-11).
run_agent_loop is not wired into any caller yet -- these tests pin its own contract directly,
independent of any surface, using fakes for the agent/stream (same convention test_api.py already
uses for iter_agent_stream's contract) and a real RunState against a tempdir workspace.

Six scenarios, one per axis this module's docstring calls out as load-bearing:
