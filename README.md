...
Even after tightening the WebSearcher/AcademicSearcher prompts to explicitly forbid returning a summary without first calling `fetch_url_to_workspace`, and after fixing the malformed-`delegate_tasks`-silently-degrading, no-dispatch, and narrated-report bugs documented above, an extensive test battery (8 live runs, 2026-07-10, spanning factual/comparative/academic/current-event queries) never once observed a real `fetch_url_to_workspace` call for a citation that later got checked — `fetched_url_count` stayed at 0 in every `_run_state.json` produced. The causal chain is directly observable in one run: asked for the latest Rust version, the model got real, current `web_search` results back (confirming the search pipeline itself genuinely reaches live information beyond training cutoff), but the returned snippets didn't happen to contain the specific version number — and rather than fetch the actual release-notes page to get it, the model fell back to confidently fabricating "Rust 1.75.0, released June 2026" (the real 1.75.0 shipped in December 2023) with entirely invented release notes. A separate academic-paper test showed the same pattern's other face: the model fabricated the paper's authors outright while, in the same response, correctly citing an unrelated real paper by title and arXiv ID — illustrating exactly why the grounding check can't distinguish the correct half from the fabricated half without a real fetch, and must reject both. The grounding check catches this every time it's been observed — quarantining, forcing a correction attempt, and (new) salvaging a narrated-but-unwritten report with a clear disclaimer rather than losing it — but it does not *prevent* the underlying behavior, only catch and disclose it. This looks like a genuine instruction-following limit of this model class rather than a prompt-wording problem — further prompt iteration didn't change it in testing, and it persisted across every model trialed as Planner (`mistral-nemo:12b`, `devstral:24b`, `llama3-groq-tool-use:8b`, `mistral:7b-instruct-v0.3`) — see "Model choice" below. The two real next steps, in order of effort: a structural fix that refuses to accept a Searcher's findings without a real fetch for non-trivial queries (tracked in `ROADMAP.md`); or the RL fine-tuning direction noted in the original plan doc, which targets this exact class of problem at its root instead of working around it.
