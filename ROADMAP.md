...
- **`context_budget_chars` blind spot for the one remaining Planner-escalated problem**
  (see the FindingsWriter entry above) — `not_delegated` is now the ONLY problem still injecting a
  nudge into the Planner's own `current_input` (as of 2026-07-14, `missing_findings`/
  `findings_ungrounded` moved to FindingsWriter's fresh-context dispatch, same as Builder already
  had for `final_report.md` problems); that one nudge is still uncounted by the context-budget
  guard. Lower priority (rare, terminal problem) but a real residual gap.
- **Planner doesn't know a Write→Review→Fix cycle just ran, and can keep re-delegating past it**
  (found live 2026-07-14, same session that added FindingsWriter — see its entry above for the
  full trace). After `run_completion_check` dispatches FindingsWriter or Builder, it returns
  `current_input` UNCHANGED (the whole point — no context growth) — but that also means the
  Planner's own next turn has zero signal that a fix cycle happened at all, or whether it actually
  worked. Confirmed live: when Builder kept re-committing the identical `claim_unsupported`
  mistake across separate corrective dispatches, the Planner reacted each time by delegating MORE
  research (`delegate_tasks` has its own quota, independent of `max_completion_check_attempts`),
  even though the actual problem was a downstream citation/authoring error, not a research gap —
  turning a trivially simple factual query into a 25-minute, 35-URL, 8-attempt run before the
  completion-check budget finally forced the quarantine-restore safety net to end it (which DID
  work correctly — a real, honestly-labeled report, not a silent failure). Candidate fix, not yet
  evaluated: after a Write→Review→Fix dispatch, inject a short summary of the outcome (e.g. "a
  FindingsWriter/Builder pass just ran and {succeeded / was flagged and re-fixed}") into the
  Planner's next turn so it can decide whether more delegation is actually warranted, instead of
  operating blind. Needs care not to reintroduce the context-growth problem this whole mechanism
  exists to avoid — a one-line status note, not the full critique text, is probably the right size.
